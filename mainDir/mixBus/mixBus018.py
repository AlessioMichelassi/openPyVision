...
class MixBusWorker(QRunnable):
    ...
    def run(self):
        ...
        try:
            ...
            elif self.effect_type == MIX_TYPE.WIPE_TOP_TO_BOTTOM:
                mixed_frame = self.wipeTopToBottom(self.prw_frame, self.prg_frame)
            elif self.effect_type == MIX_TYPE.WIPE_BOTTOM_TO_TOP:
                mixed_frame = self.wipeBottomToTop(self.prw_frame, self.prg_frame)
            elif self.effect_type in [MIX_TYPE.WIPE_STINGER1, MIX_TYPE.WIPE_STINGER2]:
                mixed_frame = self.stinger(self.prw_frame, self.prg_frame)
            else:
    ...
    def wipeTopToBottom(self, preview_frame, program_frame):
        """
        Perform a top-to-bottom wipe transition between preview and program frames.

        :param preview_frame: Il frame di anteprima (preview).
        :param program_frame: Il frame del programma (program).
        :return: Il frame con l'effetto di wipe applicato.
        """
        wipe_position = int(self.wipe_positions["topToBottom"][int(self.wipe)])
        wipe_frame = program_frame.copy()
        wipe_frame[:wipe_position, :] = preview_frame[:wipe_position, :]
        return wipe_frame
    ...
    def wipeBottomToTop(self, preview_frame, program_frame):
        """
        Perform a bottom-to-top wipe transition between preview and program frames.

        :param preview_frame: Il frame di anteprima (preview).
        :param program_frame: Il frame del programma (program).
        :return: Il frame con l'effetto di wipe applicato.
        """
        wipe_position = int(self.wipe_positions["bottomToTop"][int(self.wipe)])
        wipe_frame = program_frame.copy()
        wipe_frame[wipe_position:, :] = preview_frame[wipe_position:, :]
        return wipe_frame
    ...
    def stinger(self, preview_frame, program_frame):
        """
        Applica l'animazione Stinger per la transizione tra preview e program frames.

        :param preview_frame: Il frame in anteprima (preview).
        :param program_frame: Il frame del programma (program).
        :return: Il frame risultante con l'effetto Stinger applicato.
        """
        stingerIndex = 1 if self.effect_type == MIX_TYPE.WIPE_STINGER1 else 2

        # Recupera la posizione del wipe corrente per lo stinger
        if self.wipe < len(self.wipe_positions["stinger"][stingerIndex]):
            wipe_position = int(self.wipe_positions["stinger"][stingerIndex][self.wipe])
        else:
            wipe_position = int(self.wipe_positions["stinger"][stingerIndex][-1])

        # Prendi i frame dello stinger e la maschera inversa
        stinger_frame = self.stingers[stingerIndex][0][wipe_position]
        inv_mask = self.stingers[stingerIndex][1][wipe_position]

        if self.wipe < len(self.stingers[stingerIndex][0]) // 2:
            # Prima metà dell'animazione: nascondi il programma e mostra lo stinger
            program_masked = cv2.multiply(program_frame, inv_mask, dtype=cv2.CV_8U)
            result = cv2.add(stinger_frame, program_masked)
            return np.ascontiguousarray(result)
        else:
            # Seconda metà dell'animazione: nascondi il preview e mostra lo stinger
            preview_masked = cv2.multiply(preview_frame, inv_mask, dtype=cv2.CV_8U)
            result = cv2.add(stinger_frame, preview_masked)
            return np.ascontiguousarray(result)

    def applyStill(self, program_frame, still_frame):
        """
        Applica la dissolvenza su una still image (immagine statica).

        :param program_frame: Il frame attuale del programma.
        :param still_frame: La still image (immagine statica).
        :return: Il frame risultante.
        """
        return cv2.addWeighted(still_frame, self.fade, program_frame, 1 - self.fade, 0)


class MixBus018(QObject):
    """
    ...
    def getWipePositions(self):
        ...
        :return: Dictionary containing wipe positions for all wipe directions.
        """
        return {
            "leftToRight": self._wipe_position_leftToRight_list,
            "rightToLeft": self._wipe_position_rightToLeft_list,
            "topToBottom": self._wipe_position_topToBottom_list,
            "bottomToTop": self._wipe_position_bottomToTop_list,
            "stinger": self._wipe_position_stinger_list
        }
    ...
    def parseTallySignal(self, tally_data):
        ...
        if cmd == 'cut':
            ...
            position = tally_data.get('position')
            stingerObject = self.videoHub.getStingerPlayer(position)
            if stingerObject:
                _premultOverlay, _inv_alphas = stingerObject.getFrames()
                self.setStinger(position, _premultOverlay, _inv_alphas)
